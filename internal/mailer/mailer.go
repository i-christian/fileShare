...
func (m *Mailer) Send(recipient, templatFile string, data any) error {
	textTmpl, err := tt.New("").ParseFS(templateFS, "templates/"+templatFile)
	if err != nil {
		return err
	}
	...
	htmlTmpl, err := ht.New("").ParseFS(templateFS, "templates/"+templatFile)
	if err != nil {
		return err
	}
